...
export interface AuthContextType {
  ...
  login: (email: string, password: string) => Promise<{ success: boolean; error?: string; attemptsLeft?: number }>;
  register: (userData: {
    email: string;
    password: string;
    firstName: string;
  ...
  logout: () => Promise<void>;
  updateProfile: (updates: Partial<Pick<User, 'firstName' | 'lastName' | 'organization'>>) => Promise<{ success: boolean; error?: string }>;
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      if (AuthService.isAuthenticated()) {
        const { user: authUser, error } = await AuthService.getCurrentUserProfile();
